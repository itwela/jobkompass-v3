...
export async function GET(request: NextRequest) {
  ...
  if (!convexToken) {
    return NextResponse.redirect(new URL("/app?mode=settings&gmail_error=no_session", request.url));
  }

  let tokens: { access_token?: string | null; refresh_token?: string | null; expiry_date?: number | null };
  let userInfoEmail: string | null | undefined;
  try {
    ...
    return NextResponse.redirect(
      new URL(`/app?mode=settings&gmail_error=token_exchange_failed&detail=${encodeURIComponent(reason)}`, request.url)
    );
  }
  ...
  try {
    ...
    console.error("Gmail connectAccount failed:", error);
    const reason = error instanceof Error ? error.message : String(error);
    return NextResponse.redirect(
      new URL(`/app?mode=settings&gmail_error=connect_failed&detail=${encodeURIComponent(reason)}`, request.url)
    );
  }
